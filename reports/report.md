...
**Muhammad Osama Ahmad Ghuman, 24179282, 7PAM2032-0509-2025, 08 August 2026**
...
|---|---|---|---|---|
| Chronos (zero-shot) | **36.23** | 74.40 | **0.678** | −29.87 |
| SARIMAX (conditional) | 37.85 | 66.41 | 0.708 | −5.14 |
| Feature model | 37.89 | **64.99** | 0.709 | −4.00 |
| SARIMAX (operational) | 38.43 | 66.35 | 0.719 | −3.93 |
| Weekly seasonal naive | 43.46 | 81.41 | 0.813 | −13.16 |
| Daily seasonal naive | 48.31 | 85.57 | 0.904 | 1.75 |
| Mean | 50.26 | 74.94 | 0.941 | −3.29 |
| Naive | 85.55 | 110.39 | 1.601 | 50.98 |
...
**The metric chosen determines the winner.** Chronos leads on MAE and MASE and
comes seventh of nine on RMSE; the feature model leads on RMSE and comes third
on MASE; the flat mean forecast is sixth on MAE but fourth on RMSE. No single
model is best on both. Section 8.3 explains the Chronos case — a median point
summary on a right-skewed target — and Section 5 explains the mean forecast:
refusing to guess the timing of consumption spikes avoids the two-sided errors
that squared-error loss punishes hardest. Reporting one metric alone would in
each case support a conclusion the other contradicts.
...
**4. Does the foundation model outperform the simpler models?** On MASE, yes — 0.678 against 0.708 for
conditional SARIMAX, achieved with no covariates and no training on this
dwelling. On RMSE, emphatically not: 74.40 against 66.41, essentially tied with
a flat mean forecast. Its bias of −29.87 is six times any other model's. As
Section 8.3 sets out, this pattern follows from summarising a predictive
distribution by its median on a right-skewed target, so the MASE advantage
partly measures a difference in summary statistic rather than in forecasting
ability. The defensible conclusion is that zero-shot transfer performs
**comparably** to models fitted on four months of this dwelling's history —
which is notable — but not better.
